...
const run = Effect.fn('Lark.run')(function*() {
  ...
  while (true) {
    yield* Effect.gen(function*() {
      ...
      Effect.catch(() => context.writeState('check_failed', {})), Effect.catch(() => Effect.void), current.lock.withPermit
    )
...
export const lark = defineIntegration({
  ...larkMetadata, actions, resources, install: installConfirmed, inspect, run,
  onActionCallback: (actionId) => actionId === 'install' ? installConfirmed() : connect()
})
