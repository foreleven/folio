...
const authorizationActions = Effect.fn('LarkIntegration.authorizationActions')(function*(url?: string): Effect.fn.Return<readonly IntegrationAction[], IntegrationError> {
  if (!url) return []
...
function integrationState(phase: LarkAuthSnapshot['phase']): CheckResult {
  switch (phase) {
    case 'app_missing': return result('app_required', 'connect')
    case 'app_rejected': return result('app_authorization_required', 'connect')
    case 'app_recovering': return result('recovering')
    case 'user_missing':
    case 'user_rejected': return result('login_required', 'connect')
    case 'user_recovering': return result('recovering')
...
const make = Effect.fn('LarkIntegration.make')(function*() {
  ...
  const check = Effect.fn('LarkIntegration.check')(function*() {
    yield* auth.check()
  }, Effect.annotateLogs({ integration: 'lark', subsystem: 'integration' }), Effect.withLogSpan('lark.integration.check'))

  /** Read-only inspection maps durable auth facts into UI state and never triggers remote work. */
  const inspect = Effect.fn('LarkIntegration.inspect')(function*(): Effect.fn.Return<CheckResult, unknown, IntegrationContext | FileSystem.FileSystem | ChildProcessSpawner.ChildProcessSpawner> {
  ...
  const install = Effect.fn('LarkIntegration.install')(function*() {
    const context = yield* IntegrationContext
    yield* Effect.logInfo('Lark installation started')
    yield* auth.exclusive(Effect.gen(function*() {
  ...
  Effect.annotateLogs({ integration: 'lark', subsystem: 'integration' }), Effect.withLogSpan('lark.integration.install'))

  const connect = Effect.fn('LarkIntegration.connect')(function*() {
    const context = yield* IntegrationContext
  ...
  Effect.annotateLogs({ integration: 'lark', subsystem: 'integration' }), Effect.withLogSpan('lark.integration.connect'))

  /** Serializes background auth writes with setup/OAuth while leaving base-layer live inspection nonblocking. */
  const run = Effect.fn('LarkIntegration.run')(function*() {
    const context = yield* IntegrationContext
    yield* auth.exclusive(migratePrivateState(context.directory))
    while (true) {
      yield* Effect.gen(function*() {
        ...
        Effect.catch(() => context.writeState('check_failed', {})),
        Effect.catch(() => Effect.void)
      )
      yield* Effect.sleep(yield* auth.nextMaintenance().pipe(Effect.catch(() => Effect.succeed(30_000))))
    }
  }, Effect.ensuring(auth.release()), Effect.annotateLogs({ integration: 'lark', subsystem: 'integration' }),
  Effect.withLogSpan('lark.integration.run'))
