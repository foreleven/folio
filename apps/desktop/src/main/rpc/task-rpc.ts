...
export const TaskRpcHandlersLive = TaskRpcs.toLayer(
  Effect.gen(function* () {
    ...
    return TaskRpcs.of({
      ...
      'routines.scheduleSettings': ({ vaultId }) => service.scheduleSettings(vaultId),
      'routines.saveSchedule': ({ vaultId, input }) => service.saveSchedule(vaultId, input),
      'routines.removeSchedule': ({ vaultId, routineId, revision }) => service.removeSchedule(vaultId, routineId, revision),
      'routines.setTimeZone': ({ vaultId, timeZone, expected }) => service.setScheduleTimeZone(vaultId, timeZone, expected),
      'routines.wakeups': ({ vaultId, routineId }) => service.routineWakeups(vaultId, routineId),
      'routines.triggers': ({ vaultId, routineId }) => service.routineTriggers(vaultId, routineId),
      'routines.list': ({ vaultId }) => service.listRoutines(vaultId),
      'routines.save': ({ vaultId, input }) => service.saveRoutine(vaultId, input),
      'routines.startTask': ({ vaultId, input }) => service.startRoutineTask(vaultId, input),
      'routines.createTask': ({ vaultId, input }) => service.createRoutineTask(vaultId, input),
