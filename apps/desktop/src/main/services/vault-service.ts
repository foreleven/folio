...
export class VaultService extends Context.Service<VaultService, {
  /** Resolves an existing directory, saves its global identity, and ensures its local settings file. */
  ...
  static readonly layer = Layer.effect(VaultService, Effect.gen(function*() {
    ...
    /** Uses the global index for identity; an incomplete settings initialization is retryable with the same ID. */
