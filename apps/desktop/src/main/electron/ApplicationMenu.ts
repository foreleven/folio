...
export const ApplicationMenuLive = Layer.effectDiscard(
  Effect.gen(function* () {
    ...
    yield* Effect.forkScoped(
      Effect.forever(
        Queue.take(requests).pipe(
          Effect.flatMap(
            Effect.fn('ApplicationMenu.handle')(function* (request) {
              if (request === 'settings') yield* settings.toggle
              else if (request === 'new') yield* windows.open
              else yield* vaults.open
            })
          ),
          Effect.tapError((error) => Effect.logError('Failed to handle menu action', error)),
          Effect.catch((error) =>
            Effect.sync(() =>
              dialog.showErrorBox(
    ...
    const settingsItem: MenuItemConstructorOptions = {
      ...
      click: () => {
        Queue.offerUnsafe(requests, 'settings')
      }
    }
        ...
        submenu: [
          {
            ...
            click: () => {
              Queue.offerUnsafe(requests, 'new')
            }
          },
          {
            label: 'Open Vault…',
            ...
            click: () => {
              Queue.offerUnsafe(requests, 'open')
            }
          },
          { type: 'separator' },
