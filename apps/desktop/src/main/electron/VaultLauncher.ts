...
export class VaultLauncher extends Context.Service<VaultLauncher, {
  ...
  static readonly layer = Layer.effect(VaultLauncher, Effect.gen(function*() {
    ...
    const openDirectory = Effect.fn('VaultLauncher.openDirectory')(function*(directory: string, sourceWindowId?: number) {
      ...
      yield* windows.openVault(vault, sourceWindowId).pipe(Effect.mapError((cause) =>
    ...
    const openExisting = Effect.fn('VaultLauncher.openExisting')(function*(id: string) {
      ...
      return yield* Effect.gen(function*() {
        const current = yield* config.get.pipe(Effect.mapError((cause) =>
          new VaultError({ message: 'Could not read the vault index.', cause })
        ))
    ...
    return VaultLauncher.of({ open, openExisting })
  }))
}
