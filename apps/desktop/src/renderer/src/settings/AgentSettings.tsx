...
export function AgentSettings(): React.JSX.Element {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <p className="mb-3 text-support text-muted-foreground">{chinese ? '在此配置 Pi 的 Provider 和模型。Codex 使用本地 Codex 配置，可在创建任务时选择。' : 'Configure Pi providers and models here. Codex uses its local configuration and can be selected when creating a task.'}</p>
        <div role="group" aria-label={chinese ? '选择 Agent' : 'Choose agent'} className="grid grid-cols-2 gap-3">
          ...
          <button type="button" disabled aria-pressed="false" className="flex items-center gap-3 rounded-xl border border-dashed p-4 text-left opacity-50">
            ...
            <span><span className="block text-ui font-medium">Codex</span><span className="text-support text-muted-foreground">{chinese ? '使用本地配置' : 'Uses local configuration'}</span></span>
          </button>
        </div>
      </div>
      <ModelsSettings />
