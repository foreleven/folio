...
export function IntegrationsSettings(): React.JSX.Element {
  ...
  const result = useAtomValue(IntegrationRpcClient.integrations)
  const refresh = useAtomRefresh(IntegrationRpcClient.integrations)
  ...
  if (result._tag === 'Failure')
    return (
      <div role="alert" className="p-3">
        <p className="mb-3 text-ui">{text.loadFailed}</p>
      ...
      <div className="flex flex-col gap-1">
        {result.value.map((integration) => (
          <IntegrationCard
            ...
            locale={locale}
            pending={pending.includes(integration.id)}
            error={errors.includes(integration.id)}
            onInstall={() => {
              void submit(integration.id, () => install({ payload: { id: integration.id } })).catch(() => undefined)
            }}
            onInspect={() => {
