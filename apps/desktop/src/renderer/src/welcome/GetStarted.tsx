...
export function GetStarted({ opening, onOpen }: GetStartedProps): React.JSX.Element {
  ...
  return (
    <section className="min-w-0" aria-labelledby="get-started-heading">
      ...
      <Button
        variant="ghost"
        className="h-11 w-full justify-start gap-3 rounded-md px-3 text-sm font-normal sm:px-4 sm:text-base"
        disabled={opening !== null}
        aria-label={opening === 'picker' ? (chinese ? '正在打开' : 'Opening') : chinese ? '打开知识库' : 'Open Vault'}
        aria-describedby="open-vault-guidance"
        onClick={() => void onOpen()}
      >
        {opening === 'picker' ? (
          <LoaderCircleIcon
        ...
        <span>
          {opening === 'picker' ? (chinese ? '正在打开…' : 'Opening…') : chinese ? '打开知识库' : 'Open Vault'}
        </span>
