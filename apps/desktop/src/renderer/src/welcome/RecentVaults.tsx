...
function RecentVaultRow({ vault, opening, onOpen }: RecentVaultRowProps): React.JSX.Element {
  ...
  return (
    <li>
      <Button
        variant="ghost"
        className="h-11 w-full justify-start gap-3 rounded-md px-3 text-left sm:px-4"
        aria-label={`${vault.name}: ${vault.path}`}
        disabled={opening !== null}
        onClick={() => void onOpen(vault.id)}
      >
        ...
        <span className="grid min-w-0 flex-1 gap-0.5 sm:grid-cols-[minmax(0,0.7fr)_minmax(0,1fr)] sm:items-center sm:gap-6">
          ...
          <span className="truncate font-mono text-xs font-normal text-muted-foreground sm:text-right">
            {vault.path}
          </span>
        </span>
        {busy ? (
...
function RecentVaultSkeleton(): React.JSX.Element {
  return (
    <div role="status" aria-label="Loading vaults">
      {[0, 1, 2].map((index) => (
        <div className="flex h-11 items-center gap-3 px-3 sm:px-4" key={index}>
  ...
  return (
    <section className="min-w-0" aria-labelledby="recent-vaults-heading">
      <div className="mb-3 flex w-full items-center gap-4">
        <h2
          className={cn(
            'shrink-0 font-mono text-xs font-medium text-muted-foreground',
            !chinese && 'tracking-[0.04em]'
          )}
      ...
      {config._tag === 'Failure' ? (
        ...
        <div className="flex h-11 items-center px-3 sm:px-4">
          <p className="m-0 text-sm text-muted-foreground">
            {chinese ? '还没有知识库。打开一个文件夹，从这里开始。' : 'No vaults yet. Open a folder to get started.'}
          </p>
        </div>
      ) : (
        <ul className="list-none p-0">
          {vaults.map((vault) => (
            <RecentVaultRow key={vault.id} vault={vault} opening={opening} onOpen={onOpen} />
          ))}
        </ul>
      )}
    </section>
